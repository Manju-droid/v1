...
import { validateCreateDebateRequest, validateUpdateDebateRequest, validateJoinDebateRequest, VALID_DEBATE_DURATIONS, } from '../src/services/debate.service';
...
describe('validateJoinDebateRequest', () => {
    ...
    it('should accept valid sides', () => {
        const validSides = ['AGREE', 'DISAGREE'];
        validSides.forEach((side) => {
            const request = { userId: 'user1', side };
            const result = validateJoinDebateRequest(request);
            expect(result.valid).toBe(true);
        });
    });
