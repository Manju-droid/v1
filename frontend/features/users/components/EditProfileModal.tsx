...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({ user, onClose, onUpdate }) => {
    ...
    // safer parsing of DOB: split by 'T' to get date part directly, avoiding timezone shifts from new Date()
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // If selecting a gender and using a default avatar or changing gender,
            // clear the avatarUrl so the new Ghibli one (DiceBear adventurer) takes over dynamically.
            let avatarUrlToSave = user.avatarUrl;
            // Clear avatar if gender changed OR if using legacy styles (ui-avatars, avataaars without traits, or the previous adventurer style)
            if (gender !== user.gender || (gender && (
                user.avatarUrl.includes('ui-avatars.com') ||
                user.avatarUrl.includes('avataaars') ||
                user.avatarUrl.includes('adventurer') // Force migrate away from adventurer
            ))) {
                avatarUrlToSave = '';
            }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
                ...
                className="w-full max-w-lg bg-gray-900 border border-white/[0.1] rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]"
            >
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="relative w-20 h-20 rounded-full overflow-hidden bg-gray-800">
                                ...
                                <Avatar
                                    user={{
                                        ...user,
                                        avatarUrl: gender !== user.gender ? '' : user.avatarUrl,
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2 bg-black/20 border border-white/[0.1] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all [color-scheme:dark]"
                            />
                        </div>
