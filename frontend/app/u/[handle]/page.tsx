...
export default function ProfilePage() {
  ...
  // Fetch user points when profile loads (replaces useUserStore)
  useEffect(() => {
    const loadUserPoints = async () => {
      if (!profile?.id) return;
      try {
        const user = await userAPI.getById(profile.id);
        setUserPoints({
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        // Use follow status from profile data
        const finalFollowStatus = profileData.isFollowing;
  ...
  const refreshSavedPosts = async () => {
    ...
    try {
      ...
      // Small delay to ensure backend has completed the save/unsave operation
  ...
  const handlePostDelete = async (postId: string) => {
    ...
    if (isOwnProfile && currentUser?.id) {
      try {
        ...
        if (updatedUser) {
          setUserPoints({
            tier: updatedUser.tier || 'SILVER',
            points: updatedUser.points || 0,
            subscriptionActive: updatedUser.subscriptionActive || false,
            temporarilyMuted: updatedUser.temporarilyMuted || false,
            mutedUntil: updatedUser.mutedUntil ? (updatedUser.mutedUntil instanceof Date ? updatedUser.mutedUntil.toISOString() : updatedUser.mutedUntil) : undefined,
  ...
  useEffect(() => {
    ...
    const updateCounts = async () => {
      ...
      try {
        ...
        if (updatedProfile) {
          // Use follow status from profile data
          const finalFollowStatus = updatedProfile.isFollowing;
